...
public class SequenceAligner {
  ...
  private void fillCache() {
	  ...
	  for (int i = 1; i <= n; i++)
		  cache[i][0] = new Result(i * getJudge().getGapCost(), Direction.UP);
	  for (int j = 1; j <= m; j++)
		  cache[0][j] = new Result(j * getJudge().getGapCost(), Direction.LEFT);
	  
	  //rest of the cache
	  for (int i = 1; i < n + 1; i++) {
		  for (int j = 1; j < m + 1; j++) {
			  //M (diag)
			  int diagScore = getJudge().score(x.charAt(i - 1), y.charAt(j - 1));
			  int resultM = cache[i-1][j-1].getScore() + diagScore;
			  //I (left)
			  int resultI = cache[i][j - 1].getScore() + getJudge().getGapCost();
			  //D (up)
			  int resultD = cache[i - 1][j].getScore() + getJudge().getGapCost();
			  
			  int max = resultM;
			  Direction resultDir = Direction.DIAGONAL;
			  
			  
			  if (max < resultI) {
				  max = resultI;
				  resultDir = Direction.LEFT;
			  }
			  
			  if (max < resultD) {
				  max = resultD;
				  resultDir = Direction.UP;
			  }
			  
			  cache[i][j] = new Result(max, resultDir);
		  }
	  }
  } 
  ...
  private void traceback() {
	...
	int i = n;
	int j = m;
    Result traceResult = cache[n][m];
    Direction traceDir = Direction.NONE;
    
    while(i > 0 && j > 0) {
    	cache[0][0].markPath(); // First result
    	cache[i][j].markPath(); // last result
    	traceDir = cache[i][j].getParent();
    	if (traceDir == Direction.DIAGONAL) {
    		i--;
    		j--;
    		alignedX = x.charAt(i) + alignedX;
    		alignedY = y.charAt(j) + alignedY;
    	}
    	if (traceDir == Direction.LEFT) {
    		j--;
    		alignedY = y.charAt(j) + alignedY;
